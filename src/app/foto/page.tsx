...
import { albumMeta, albums } from "@/lib/photos";
...
export const metadata: Metadata = {
  ...
  twitter: { card: "summary_large_image", title: "Photography", description: DESCRIPTION },
};
          ...
          <p className="text-muted-foreground">
            ...
            {photoCount > 0 && (
              <span className="text-muted-foreground/70">
                {" "}
                {albums.length} {albums.length === 1 ? "album" : "albums"}, {photoCount} frames.
              </span>
            )}
          </p>
        </div>
      ...
      {albums.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
          {albums.map((album, id) => {
            ...
            return (
              <BlurFade key={album.slug} delay={BLUR_FADE_DELAY * 2 + id * 0.04}>
                <Link
                ...
                >
                  <div className="relative aspect-4/3 overflow-hidden rounded-lg border border-border bg-muted shadow-sm transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:shadow-md">
                    <img
                  ...
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <h2 className="text-sm font-semibold leading-snug tracking-tight transition-colors group-hover:text-foreground">
                      {album.title}
                    </h2>
